#Yvonne Vasquez
#Date: 7/23/19
#title: ss_cluster_outlier_genes_v4
...
filtered_cancer_druggable_genes <- expression_data_filtered_genes_anno %>%
  mutate('Is_a_cancer_gene?' = gene %in% cancer_gene_names,  'Is a druggable gene?' = gene %in% druggable_gene_names$gene) 
...
multi_sample_outliers <- filtered_cancer_druggable_genes %>%
  group_by(gene) %>%
